...
MIN_T   <- 50
suppressMessages({library(ggplot2); library(effsize)})
set.seed(1)
...
## Cliff's delta (M vs F; >0 = higher in MALES) via the effsize package.
## effsize::cliff.delta(m, f) returns the estimate and an analytic 95% CI; we take
## an approximate SE from the CI width for the inverse-variance pooling below.
cliffs <- function(m, f) {
  m <- m[!is.na(m)]; f <- f[!is.na(f)]
  if (length(m) < 2 || length(f) < 2) return(c(d=NA, lo=NA, hi=NA, se=NA))
  cd <- effsize::cliff.delta(m, f)
  lo <- unname(cd$conf.int[1]); hi <- unname(cd$conf.int[2])
  c(d=unname(cd$estimate), lo=lo, hi=hi, se=(hi - lo) / (2 * qnorm(0.975)))
}
