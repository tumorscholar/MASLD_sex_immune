## sc_validate_ramachandran.R -------------------------------------------------
## Sex validation on Ramachandran 2019 (GSE136103), human liver CD45+ cells.
## Healthy donors test the CONSTITUTIONAL male-MAIT prediction; cirrhotic donors
...
MARK <- list(
 Tcell = c("CD3D","CD3E","CD3G"), MAIT = c("SLC4A10","TRAV1-2"),
 CD8 = c("CD8A","CD8B"), Treg = c("FOXP3"),
 cDC = c("CD1C","CLEC9A","CLEC10A","FCER1A"), pDC = c("LILRA4","IL3RA","GZMB"),
 immune = c("PTPRC"))

message("Loading ", OBJ_PATH)
obj <- readRDS(OBJ_PATH); DefaultAssay(obj) <- "RNA"
md <- obj@meta.data; stopifnot(DONOR_COL %in% colnames(md))
...
is_MAIT<- is_T & anyMark(MARK$MAIT); is_CD8 <- is_T & anyMark(MARK$CD8); is_Treg <- is_T & anyMark(MARK$Treg)
...
long <- do.call(rbind, lapply(names(EXPECT), function(v) data.frame(readout=v, group=plotset$group, sex=plotset$sex, value=plotset[[v]])))
long$readout <- factor(long$readout, levels=names(EXPECT), labels=c("MAIT (%T)","Treg (%T)","CD8 (%T)","cDC (%imm)"))
p <- ggplot(long, aes(sex,value,fill=sex)) + geom_boxplot(outlier.shape=NA,alpha=.5) + geom_jitter(width=.15,size=1.4,alpha=.8) +
 facet_grid(group~readout, scales="free_y") + scale_fill_manual(values=c(M="#2c7fb8",F="#d95f0e")) +
...
ggsave(file.path(OUTDIR, paste0(DATASET,"_sex_validation.png")), p, width=9, height=5, dpi=300)
